  ...
  case Qt::Key_Space:
    ...
    if ( qItem ) {
      ...
      if ( b_WasChecked ) {
        ...
        uncheckChildren(qItem); // Uncheck all children recursively
      } else {
        qItem->setCheckState( K_CK, Qt::Checked );
        checkChildren(qItem); // Check all children recursively
      }
    }
...
void TreeWidget::uncheckChildren(QTreeWidgetItem* parentItem)
{
  int childCount = parentItem->childCount();
  for (int i = 0; i < childCount; i++) {
    QTreeWidgetItem* childItem = parentItem->child(i);
    childItem->setCheckState(K_CK, Qt::Unchecked);
    uncheckChildren(childItem); // Recursively uncheck children
  }
}

void TreeWidget::checkChildren(QTreeWidgetItem* parentItem)
{
  int childCount = parentItem->childCount();
  for (int i = 0; i < childCount; i++) {
    QTreeWidgetItem* childItem = parentItem->child(i);
    childItem->setCheckState(K_CK, Qt::Checked);
    checkChildren(childItem); // Recursively check children
  }
}
// >>>> TreeWidget::keyPressEvent
